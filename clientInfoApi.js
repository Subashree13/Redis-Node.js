...
router.post('/', (req, res) => {
    const { MsgType, OpeType, ClientId, Token, TenantId, OrderId, OMSId } = req.body;
    const key = `${TenantId}_${OMSId}_${ClientId}_${Token}:${OrderId}`;
    const body = req.body;

    if (!OpeType || !MsgType || !ClientId) {
    ...
    switch (OpeType) {
        ...
        case 102:
            ...
            const action = OpeType === 101 ? 'edited' : 'deleted';
            redis[OpeType === 101 ? 'hmset' : 'del'](key, fieldValues, (err, result) => {
                if (err) {
                    console.error('Redis error:', err);
                    return res.status(500).json({ error: 'Internal server error' });
                }
                if (result === 0 && OpeType === 102) {
